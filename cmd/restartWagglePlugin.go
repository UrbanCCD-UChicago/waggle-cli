...
var restartWagglePluginCmd = &cobra.Command{
	Use:   "restart-waggle-plugin",
	Short: d.ShortDesc(d.RestartWagglePluginDesc),
	Long:  d.RestartWagglePluginDesc,
	Run: func(_cmd *cobra.Command, args []string) {
		name := args[0]
		userCmd := fmt.Sprintf("systemctl restart waggle-plugin-%s", name)
...
func init() {
	rootCmd.AddCommand(restartWagglePluginCmd)
}
